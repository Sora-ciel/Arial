...
- Save: writes local IndexedDB first, then remote (if signed in).
- Load: signed-in users read remote first, then local fallback.
...
If you need migration, do it with an explicit one-off admin script and user consent.
